...
Bank::Bank(std::string bankName, double interest){
    ...
    if(login()){ //if sucessfully logged in
...
void Bank::displayMenu(){
    ...
    if(user->getAdmin() != '1'){ //if user is not admin, limit their choices (they can not edit/remove/see others)
        switch (option){
            case 0:
                running = false;
            break;
    ...
    }else{ //if admin, then allow all the options
        switch (option){
            case 0:
                running = false;
            break;
...
void Bank::deleteAccount(){
    ...
    if (compareStrings("error", rline)){ //if an error was returned
    ...
    if (option == 'y' || option == 'Y'){
        ...
        while(getline(FileUser, line)){ //reads line from file and puts it in variable
            ...
            if(!compareStrings(thisusername, username)){ //if the usernames match, dont copy over to temp file
                tempFile << "\n" << line;
            }
        }
...
void Bank::displaySpecificAccount(){
    ...
    if(checkAccountExists(username)){ //make sure the account they're looking for exists
        ...
        while(getline(FileUsers, line)){ 
            if(line == ""){ //if empty line, don't try to read it
                continue;
            }
            ...
            if(!compareStrings(pusername, username)){ //if the usernames do not match, continue to next line
                continue;
            }
            ...
            for (int i = 0; i < 30 - usernameLength; i++){ //adds some basic design
                std::cout << "-";
            }
...
std::string Bank::displaySpecificAccount_return(){ //displays specific account, but returns username or error
    std::cout << "Which account are you looking for? (username): ";
    std::string username;
    std::cin >> username;
...
void Bank::displayAccount(){ //display current user account
    std::cout << "\n\n------------------------------BEGIN------------------------------\n" << std::endl;
...
void Bank::withdraw(){
    ...
    if (user->getBalance() < 0){ //if user has less than $0 in account
...
bool Bank::createAccount(){ //allows user to create new account
    std::string username, password;
    char accountType;
    ...
    if(username.length() < 4){ //makes hacking user accounts more difficult
    ...
    if (!checkAccountExists(username, password)){ //if account does not exist, create a new one
